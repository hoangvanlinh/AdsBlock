...
(function _adblock_scriptlets() {
  ...
  // Disable navigator.globalPrivacyControl spoofing injected by some
  // adblocker fingerprinting scripts (hides extension presence)
  setConstant('Navigator.prototype.globalPrivacyControl', 'false');
  setConstant('navigator.globalPrivacyControl', 'false');

  // Neutralize tab-under / visibilitychange-based ad redirects.
  // Attack: user click → window.open(legit) → current tab hidden → visibilitychange
  // fires → "hidden" === document.visibilityState → redirect to ad URL.
  // Locking visibilityState to "visible" and hidden to false breaks the condition
  //   ("hidden" !== document.visibilityState) → always true → short-circuits, e(n) never called.
  // Side-effect: tab-visibility-based video/animation pausing won't work — acceptable trade-off.
  setConstant('document.visibilityState', 'visible');
  setConstant('document.hidden', 'false');

  // Neutralize Google News ad module flag
  setConstant('gnt.x.adm', '');

  // Neutralize NCBI/NIH analytics data object
  setConstant('ncbi.sg', '{}');

  // Neutralize NCBI/NIH analytics ping
  setConstant('ncbi.sg.ping', 'noopFunc');

  // Neutralize admiral ad-recovery global
  setConstant('admiral', 'noopFunc');

  // Abort ad-recovery and ad-detection inline scripts that use these
  // DOM APIs as entry points (runs at document_start before those scripts)
  abortCurrentScript('document.createElement',       'admiral');
  abortCurrentScript('document.getElementsByTagName', '.pubads');
  abortCurrentScript('document.getElementsByTagName', 'gtm.js');
  abortCurrentScript('document.createElement',       'keen-tracking');

  // Abort scripts matching stack patterns used by ad-recovery loaders
  // NOTE: '.js?' and decodeURI patterns are site-specific (not safe for global use)
  abortOnStackTrace('HTMLElement.prototype.insertBefore', '/[A-Z] .+chunks\\/\\d{4}\\./');

  // Block fetch/XHR requests to known ad-analytics endpoints
  preventFetch('stella');
  preventXhr('stella');

  // Abort scripts that read known ad-blocker detection library globals
  abortOnPropertyRead('blockAdBlock');
  abortOnPropertyRead('fuckAdBlock');
  abortOnPropertyRead('FuckAdBlock');
  abortOnPropertyRead('BlockAdBlock');

  // Block ALL window.open calls (popup / redirect ads)
  noWindowOpenIf('');

  // Block click/mousedown/mouseover handlers that call window.open OR redirect via location
  // (catches popunders + location.href/assign/replace redirect scripts)
  preventAddEventListener('/click|mousedown|mouseover|touchstart/', '/window\\.open|location\\.(href|assign|replace)/');

  // Block deferred popunder scripts: ["keydown","click"].forEach(e => addEventListener(e, () => setTimeout(c,0), true))
  // Handler body is just `setTimeout(fn, 0)` — no direct window.open/location reference — so the rule above
  // doesn't catch it.  keydown / mousedown / touchstart + setTimeout is almost exclusively a popunder pattern;
  // legitimate handlers for those events don't typically defer side-effects with setTimeout(fn,0).
  // NOTE: on mobile, setTimeout(c,0) in a touchstart/mousedown handler runs BEFORE the click event fires,
  // allowing the ad to redirect before our click-based defenses set _allowedOrigin.
  preventAddEventListener('/keydown|mousedown|touchstart/', 'setTimeout');

  // Block popstate/pageshow handlers that contain redirect code
  // (back-button ads listen to popstate then do location.href = adUrl)
  preventAddEventListener('/popstate|pageshow/', '/location\\.href|location\\.assign|location\\.replace|window\\.open/');

  // Freeze addEventListener after ALL proxy rules are installed.
  // This prevents ad scripts from overwriting our proxies back to the originals.
  // MUST be called after all preventAddEventListener() calls — if called inside
  // the function, the first call would lock the property and block later rules.
  _protectAddEventListener();

  // Strip target="_blank" from user-clicked links — prevents forced new-tab redirects
  disableNewtabLinks();

  // Strip target from dynamically injected <a>/<form> and block form redirects
  stripDynamicTargets();

  // Block programmatic .click() on cross-origin anchors entirely.
  // Real user clicks bypass HTMLElement.prototype.click, so only ad scripts
  // (which create <a href=adUrl> and call .click()) are affected.
  // For same-origin anchors, strip target="_blank" and allow the click.
  proxyApplyFn('HTMLElement.prototype.click', function (context) {
    const el = context.thisArg;
    if (el instanceof HTMLAnchorElement && el.href && el.href.startsWith('http')) {
      try {
        if (new URL(el.href).origin !== location.origin) return; // block cross-origin
      } catch(e) {}
    }
